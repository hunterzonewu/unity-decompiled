...
namespace UnityEngine
{
  ...
  public class Physics2D
  {
    ...
    public static Vector2 gravity
    {
      ...
      {
        Vector2 vector2;
        Physics2D.INTERNAL_get_gravity(out vector2);
        return vector2;
      }
      set
      {
    ...
    {
      using (List<Rigidbody2D>.Enumerator enumerator = Physics2D.m_LastDisabledRigidbody2D.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          Rigidbody2D current = enumerator.Current;
    ...
    public static RaycastHit2D Linecast(Vector2 start, Vector2 end, [DefaultValue("DefaultRaycastLayers")] int layerMask, [DefaultValue("-Mathf.Infinity")] float minDepth, [DefaultValue("Mathf.Infinity")] float maxDepth)
    {
      ...
      Physics2D.Internal_Linecast(start, end, layerMask, minDepth, maxDepth, out raycastHit);
      return raycastHit;
    }
    ...
    {
      RaycastHit2D raycastHit;
      Physics2D.Internal_Raycast(origin, direction, distance, layerMask, minDepth, maxDepth, out raycastHit);
      return raycastHit;
    }
    ...
    {
      RaycastHit2D raycastHit;
      Physics2D.Internal_CircleCast(origin, radius, direction, distance, layerMask, minDepth, maxDepth, out raycastHit);
      return raycastHit;
    }
    ...
    {
      RaycastHit2D raycastHit;
      Physics2D.Internal_BoxCast(origin, size, angle, direction, distance, layerMask, minDepth, maxDepth, out raycastHit);
      return raycastHit;
    }
    ...
    {
      RaycastHit2D raycastHit;
      Physics2D.Internal_GetRayIntersection(ray, distance, layerMask, out raycastHit);
      return raycastHit;
    }
