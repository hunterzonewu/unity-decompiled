  ...
  public class NetworkTransform : NetworkBehaviour
  {
    ...
    private void SerializeModeTransform(NetworkWriter writer)
    {
      ...
      if (this.m_SyncRotationAxis != NetworkTransform.AxisSyncMode.None)
        NetworkTransform.SerializeRotation3D(writer, this.transform.rotation, this.syncRotationAxis, this.rotationSyncCompression);
      this.m_PrevPosition = this.transform.position;
      this.m_PrevRotation = this.transform.rotation;
      this.m_PrevVelocity = 0.0f;
    }
    ...
    private void UnserializeModeTransform(NetworkReader reader, bool initialState)
    {
      ...
      else
      {
        ...
        if (this.syncRotationAxis == NetworkTransform.AxisSyncMode.None)
          return;
        this.transform.rotation = NetworkTransform.UnserializeRotation3D(reader, this.syncRotationAxis, this.rotationSyncCompression);
      }
    }
    ...
    public static void HandleTransform(NetworkMessage netMsg)
    {
      ...
      if ((UnityEngine.Object) localObject == (UnityEngine.Object) null)
      {
        if (!LogFilter.logError)
          return;
        Debug.LogError((object) "HandleTransform no gameObject");
      }
      else
      {
        ...
        if ((UnityEngine.Object) component == (UnityEngine.Object) null)
        {
          if (!LogFilter.logError)
            return;
          Debug.LogError((object) "HandleTransform null target");
        }
        else if (!component.localPlayerAuthority)
        {
          if (!LogFilter.logError)
            return;
          Debug.LogError((object) "HandleTransform no localPlayerAuthority");
        }
        else if (netMsg.conn.clientOwnedObjects == null)
        {
          if (!LogFilter.logError)
            return;
          Debug.LogError((object) "HandleTransform object not owned by connection");
        }
