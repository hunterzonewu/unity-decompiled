  ...
  internal class ModelImporterModelEditor : AssetImporterInspector
  {
    ...
    private void MaterialsGUI()
    {
      ...
      if (this.m_ImportMaterials.boolValue)
      {
        ...
        text = ModelImporterModelEditor.styles.MaterialHelpStart.text.Replace("%MAT%", ModelImporterModelEditor.styles.MaterialNameHelp[this.m_MaterialName.intValue].text) + "\n" + ModelImporterModelEditor.styles.MaterialSearchHelp[this.m_MaterialSearch.intValue].text + "\n" + ModelImporterModelEditor.styles.MaterialHelpEnd.text;
      }
      else
      ...
      foreach (object target in this.targets)
      {
        ...
        if ((double) globalScale != (double) floatValue && (double) floatValue != 0.0 && (double) globalScale != 0.0)
        {
          ...
          for (int index = 0; index < property.arraySize; ++index)
          {
            ...
            arrayElementAtIndex.FindPropertyRelative(AvatarSetupTool.sPosition).vector3Value = arrayElementAtIndex.FindPropertyRelative(AvatarSetupTool.sPosition).vector3Value * num;
          }
        }
      }
    }
    ...
    private class Styles
    {
      ...
      public GUIContent[] MaterialNameOptMain = new GUIContent[3]{ EditorGUIUtility.TextContent("By Base Texture Name"), EditorGUIUtility.TextContent("From Model's Material"), EditorGUIUtility.TextContent("Model Name + Model's Material") };
      public GUIContent[] MaterialNameOptAll = new GUIContent[4]{ EditorGUIUtility.TextContent("By Base Texture Name"), EditorGUIUtility.TextContent("From Model's Material"), EditorGUIUtility.TextContent("Model Name + Model's Material"), EditorGUIUtility.TextContent("Texture Name or Model Name + Model's Material (Obsolete)") };
      public GUIContent MaterialSearch = EditorGUIUtility.TextContent("Material Search");
      public GUIContent[] MaterialSearchOpt = new GUIContent[3]{ EditorGUIUtility.TextContent("Local Materials Folder"), EditorGUIUtility.TextContent("Recursive-Up"), EditorGUIUtility.TextContent("Project-Wide") };
