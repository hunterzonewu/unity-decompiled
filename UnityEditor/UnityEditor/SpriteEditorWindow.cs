...
// Type: UnityEditor.SpriteEditorWindow
...
namespace UnityEditor
{
  internal class SpriteEditorWindow : SpriteUtilityWindow
  {
    ...
    private Rect inspectorRect
    {
      get
      {
        return new Rect((float) ((double) this.position.width - 330.0 - 8.0 - 16.0), (float) ((double) this.position.height - 148.0 - 8.0 - 16.0), 330f, 148f);
      }
    }
    ...
    {
      if ((UnityEngine.Object) SpriteEditorWindow.s_Instance == (UnityEngine.Object) null)
        return;
    ...
    private void ApplyCacheSettingsToInspector(SerializedObject so)
    {
      ...
      {
        if (!this.textureIsDirty || !EditorUtility.DisplayDialog("Sprite Editor Window", "You have pending changes in the Sprite Editor Window.\nDo you want to apply these changes?", "Yes", "No"))
          return;
        this.DoApply(so);
      }
    }
    ...
    public void RefreshPropertiesCache()
    {
      ...
      if ((UnityEngine.Object) this.m_RectsCache != (UnityEngine.Object) null)
        this.selected = this.m_TextureImporterSprites.arraySize <= 0 ? (SpriteRect) null : this.m_RectsCache.RectAt(0);
      int width = 0;
      int height = 0;
    ...
    private void OnGUI()
    {
      ...
      if (!this.activeTextureSelected)
      {
        ...
        GUILayout.Label(SpriteUtilityWindow.Styles.s_NoSelectionWarning);
    ...
    {
      this.m_GizmoMode = !Event.current.control ? SpriteEditorWindow.GizmoMode.RectEditing : SpriteEditorWindow.GizmoMode.BorderEditing;
    ...
    private void DoToolbarGUI()
    {
      ...
      else
      {
        ...
        if (GUILayout.Button("Slice", (GUIStyle) "toolbarPopup", new GUILayoutOption[0]))
        {
        ...
        if (GUILayout.Button(new GUIContent("Trim", "Trims selected rectangle (T)"), EditorStyles.toolbarButton, new GUILayoutOption[0]) || string.IsNullOrEmpty(GUI.GetNameOfFocusedControl()) && SpriteEditorWindow.k_SpriteEditorTrim.activated)
        {
    ...
    private void DoPolygonChangeShapeWindow()
    {
      ...
      else
      {
        ...
        if (GUILayout.Button(new GUIContent("Change", "Change to the new number of sides")))
          flag = true;
        EditorGUI.EndDisabledGroup();
    ...
    private void DoPivotFields()
    {
      ...
      Vector2 customOffset = EditorGUI.Vector2Field(rect, "Custom Pivot", pivot);
      EditorGUI.EndDisabledGroup();
      if (!EditorGUI.EndChangeCheck())
        return;
    ...
    private void DoNameField()
    {
      ...
      if (string.IsNullOrEmpty(this.selected.m_OriginalName) && str != name)
    ...
    private void DoApply(SerializedObject so)
    {
      if (this.multipleSprites)
      {
        ...
        for (int index = 0; index < this.m_RectsCache.Count; ++index)
        {
          ...
          {
            stringList1.Add(spriteRect.m_OriginalName);
            stringList2.Add(spriteRect.m_Name);
          }
          ...
          SpriteEditorWindow.ApplyOutlineChanges(arrayElementAtIndex.FindPropertyRelative("m_Outline"), spriteRect);
        }
      ...
      else
      {
        ...
        SpriteEditorWindow.ApplyOutlineChanges(this.m_SpriteSheetOutline, spriteRect);
      }
    }
    ...
    private void HandleDragging()
    {
      ...
      Rect rect = SpriteEditorUtility.ClampedRect(SpriteEditorUtility.RoundedRect(SpriteEditorHandles.SliderRect(this.selected.m_Rect)), clamp, true);
      if (!EditorGUI.EndChangeCheck())
        return;
    ...
    {
      if (this.selected != null)
        return this.m_GizmoMode == SpriteEditorWindow.GizmoMode.RectEditing;
      return false;
    }
    ...
    private Vector4 ClampSpriteBorder(Vector4 border)
    {
      ...
      return new Vector4() { x = (float) Mathf.RoundToInt(Mathf.Clamp(border.x, 0.0f, Mathf.Min(width - border.z, width))), z = (float) Mathf.RoundToInt(Mathf.Clamp(border.z, 0.0f, Mathf.Min(width - border.x, width))), y = (float) Mathf.RoundToInt(Mathf.Clamp(border.y, 0.0f, Mathf.Min(height - border.w, height))), w = (float) Mathf.RoundToInt(Mathf.Clamp(border.w, 0.0f, Mathf.Min(height - border.y, height))) };
    }
    ...
    {
      SpriteEditorUtility.BeginLines(new Color(0.0f, 0.0f, 0.0f, 0.25f));
      for (int i = 0; i < this.m_RectsCache.Count; ++i)
      {
      ...
      {
        if (this.m_RectsCache.RectAt(i) != this.selected)
          SpriteEditorUtility.DrawBox(this.m_RectsCache.RectAt(i).m_Rect);
      }
      SpriteEditorUtility.EndLines();
      if (this.polygonSprite)
      {
          ...
          if (spriteRect.m_Outline.Count > 0)
          {
            ...
            for (int index1 = 0; index1 < spriteRect.m_Outline.Count; ++index1)
            {
              ...
              for (index2 = 0; index2 < spriteRect.m_Outline[index1].Count - 1; ++index2)
                SpriteEditorUtility.DrawLine((Vector3) (spriteRect.m_Outline[index1][index2] + vector2), (Vector3) (spriteRect.m_Outline[index1][index2 + 1] + vector2));
              SpriteEditorUtility.DrawLine((Vector3) (spriteRect.m_Outline[index1][index2] + vector2), (Vector3) (spriteRect.m_Outline[index1][0] + vector2));
            }
            SpriteEditorUtility.EndLines();
          }
        }
      ...
      SpriteEditorUtility.DrawBox(new Rect(rect1.xMin + 1f / this.m_Zoom, rect1.yMin + 1f / this.m_Zoom, rect1.width, rect1.height));
      SpriteEditorUtility.EndLines();
      SpriteEditorUtility.BeginLines(new Color(0.25f, 0.5f, 1f, 0.75f));
      SpriteEditorUtility.DrawBox(rect1);
      SpriteEditorUtility.EndLines();
    }
    ...
    private Rect TrimAlpha(Rect rect)
    {
      ...
      for (int yMin = (int) rect.yMin; yMin < (int) rect.yMax; ++yMin)
      {
        ...
        {
          if (this.PixelHasAlpha(xMin, yMin))
          {
    ...
    private void HandleBorderSidePointScalingSliders()
    {
      ...
      float y3 = num1;
      this.HandleBorderPointSlider(ref x1, ref y3, MouseCursor.ResizeHorizontal, false, dragBorderdot, dragBorderDotActive, color);
      float y4 = num1;
      this.HandleBorderPointSlider(ref x2, ref y4, MouseCursor.ResizeHorizontal, false, dragBorderdot, dragBorderDotActive, color);
      float x3 = num2;
      this.HandleBorderPointSlider(ref x3, ref y1, MouseCursor.ResizeVertical, false, dragBorderdot, dragBorderDotActive, color);
      x3 = num2;
      this.HandleBorderPointSlider(ref x3, ref y2, MouseCursor.ResizeVertical, false, dragBorderdot, dragBorderDotActive, color);
      if (EditorGUI.EndChangeCheck())
      {
    ...
    private void HandleBorderCornerScalingHandles()
    {
      ...
      float y2 = rect.yMin + border.y;
      EditorGUI.BeginChangeCheck();
      this.HandleBorderPointSlider(ref x1, ref y1, MouseCursor.ResizeUpLeft, (double) border.x < 1.0 && (double) border.w < 1.0, dragBorderdot, dragBorderDotActive, color);
      this.HandleBorderPointSlider(ref x2, ref y1, MouseCursor.ResizeUpRight, (double) border.z < 1.0 && (double) border.w < 1.0, dragBorderdot, dragBorderDotActive, color);
      this.HandleBorderPointSlider(ref x1, ref y2, MouseCursor.ResizeUpRight, (double) border.x < 1.0 && (double) border.y < 1.0, dragBorderdot, dragBorderDotActive, color);
      this.HandleBorderPointSlider(ref x2, ref y2, MouseCursor.ResizeUpLeft, (double) border.z < 1.0 && (double) border.y < 1.0, dragBorderdot, dragBorderDotActive, color);
      if (EditorGUI.EndChangeCheck())
      {
        Undo.RegisterCompleteObjectUndo((UnityEngine.Object) this.m_RectsCache, "Scale sprite border");
        border.x = x1 - rect.xMin;
    ...
    {
      Rect[] spriteRectangles = InternalSpriteUtility.GenerateGridSpriteRectangles(this.m_Texture, offset, size, padding);
      bool flag = true;
      if (spriteRectangles.Length > 1000 && !EditorUtility.DisplayDialog("Creating multiple sprites", "Creating " + (object) spriteRectangles.Length + " sprites. \nThis can take up to several minutes.", "Ok", "Cancel"))
        flag = false;
      if (flag)
      {
    ...
    public void GeneratePolygonOutline(int sides)
    {
      for (int i = 0; i < this.m_RectsCache.Count; ++i)
      {
        SpriteRect spriteRect = this.m_RectsCache.RectAt(i);
        List<Vector2> vector2List = new List<Vector2>();
        vector2List.AddRange((IEnumerable<Vector2>) UnityEditor.Sprites.SpriteUtility.GeneratePolygonOutlineVerticesOfSize(sides, (int) spriteRect.m_Rect.width, (int) spriteRect.m_Rect.height));
    ...
    private List<Rect> SortRects(List<Rect> rects)
    {
      ...
      while (rects.Count > 0)
      {
        ...
        Rect sweepRect = new Rect(0.0f, rect.yMin, (float) this.m_Texture.width, rect.height);
    ...
    private List<Rect> RectSweep(List<Rect> rects, Rect sweepRect)
    {
      ...
      using (List<Rect>.Enumerator enumerator = rectList.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          Rect current = enumerator.Current;
    ...
    private SpriteRect GetExistingOverlappingSprite(Rect rect)
    {
      for (int i = 0; i < this.m_RectsCache.Count; ++i)
      {
        if (this.Overlap(this.m_RectsCache.RectAt(i).m_Rect, rect))
          return this.m_RectsCache.RectAt(i);
      }
      return (SpriteRect) null;
    }
    ...
    {
      if ((double) a.xMin < (double) b.xMax && (double) a.xMax > (double) b.xMin && (double) a.yMin < (double) b.yMax)
        return (double) a.yMax > (double) b.yMin;
      return false;
    }
    ...
    private bool MouseOnTopOfInspector()
    {
      if (this.selected == null)
        return false;
      return this.inspectorRect.Contains(GUIClip.Unclip(Event.current.mousePosition) + new Vector2(0.0f, -22f));
    }
    ...
    private bool PixelHasAlpha(int x, int y)
    {
      ...
      if (SpriteEditorWindow.s_AlphaPixelCache == null)
      {
        ...
        Color32[] pixels32 = this.m_Texture.GetPixels32();
        for (int index = 0; index < pixels32.Length; ++index)
          SpriteEditorWindow.s_AlphaPixelCache[index] = (int) pixels32[index].a != 0;
      }
      int index1 = y * this.m_Texture.width + x;
      return SpriteEditorWindow.s_AlphaPixelCache[index1];
    }
