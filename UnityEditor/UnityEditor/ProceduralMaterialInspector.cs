...
namespace UnityEditor
{
  ...
  internal class ProceduralMaterialInspector : MaterialEditor
  {
    ...
    internal void Apply()
    {
      using (List<ProceduralMaterialInspector.ProceduralPlatformSetting>.Enumerator enumerator = this.m_PlatformSettings.GetEnumerator())
      {
    ...
    public void BuildTargetList()
    {
      ...
      this.m_PlatformSettings.Add(new ProceduralMaterialInspector.ProceduralPlatformSetting(this.targets, string.Empty, BuildTarget.StandaloneWindows, (Texture2D) null));
      using (List<BuildPlayerWindow.BuildPlatform>.Enumerator enumerator = validPlatforms.GetEnumerator())
      {
        while (enumerator.MoveNext())
        {
          ...
          this.m_PlatformSettings.Add(new ProceduralMaterialInspector.ProceduralPlatformSetting(this.targets, current.name, current.DefaultTarget, current.smallIcon));
        }
      }
    }
    ...
    {
      ProceduralMaterialInspector.ProceduralPlatformSetting platformSetting = this.m_PlatformSettings[EditorGUILayout.BeginPlatformGrouping(BuildPlayerWindow.GetValidPlatforms().ToArray(), this.m_Styles.defaultPlatform) + 1];
      ProceduralMaterialInspector.ProceduralPlatformSetting proceduralPlatformSetting = platformSetting;
      bool flag = true;
      ...
      if (EditorGUI.EndChangeCheck() && proceduralPlatformSetting.isDefault)
      {
        using (List<ProceduralMaterialInspector.ProceduralPlatformSetting>.Enumerator enumerator = this.m_PlatformSettings.GetEnumerator())
        {
          while (enumerator.MoveNext())
          {
            ProceduralMaterialInspector.ProceduralPlatformSetting current = enumerator.Current;
