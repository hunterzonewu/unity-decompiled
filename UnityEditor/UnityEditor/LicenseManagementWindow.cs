...
// Type: UnityEditor.LicenseManagementWindow
...
namespace UnityEditor
{
  internal sealed class LicenseManagementWindow : EditorWindow
  {
    ...
    private static LicenseManagementWindow win = (LicenseManagementWindow) null;
    ...
    private static LicenseManagementWindow Window
    {
      get
      {
        if ((Object) LicenseManagementWindow.win == (Object) null)
    ...
    private void OnGUI()
    {
      ...
      if (GUILayout.Button("Check for updates", new GUILayoutOption[2]
      {
      ...
      GUILayout.Label("Checks for updates to the currently installed license. If you have purchased addons you can install them by pressing this button (Internet access required)");
      GUILayout.EndHorizontal();
      GUILayout.Space(15f);
      GUILayout.BeginHorizontal();
      if (GUILayout.Button("Activate new license", new GUILayoutOption[2]
      {
        GUILayout.ExpandHeight(true),
        GUILayout.Width((float) LicenseManagementWindow.buttonWidth)
      }))
      ...
      GUILayout.Label("Activate Unity with a different serial number, switch to a free serial or start a trial period if you are eligible for it (Internet access required).");
      GUILayout.EndHorizontal();
      GUILayout.Space(15f);
      GUILayout.BeginHorizontal();
      if (GUILayout.Button("Return license", new GUILayoutOption[2]
      {
        GUILayout.ExpandHeight(true),
        GUILayout.Width((float) LicenseManagementWindow.buttonWidth)
      }))
        LicenseManagementWindow.ReturnLicense();
      GUILayout.Label("Return this license and free an activation for the serial it is using. You can then reuse the activation on another machine (Internet access required).");
      GUILayout.EndHorizontal();
      GUILayout.Space(15f);
      GUILayout.BeginHorizontal();
      if (GUILayout.Button("Manual activation", new GUILayoutOption[2]
      {
        GUILayout.ExpandHeight(true),
        GUILayout.Width((float) LicenseManagementWindow.buttonWidth)
      }))
        LicenseManagementWindow.ManualActivation();
      GUILayout.Label("Start the manual activation process, you can save this machines license activation request file or deploy a license file you have already activated manually.");
      GUILayout.EndHorizontal();
      GUILayout.Space(15f);
      GUILayout.BeginHorizontal();
      if (GUILayout.Button("Unity FAQ", new GUILayoutOption[2]
      {
        GUILayout.ExpandHeight(true),
        GUILayout.Width((float) LicenseManagementWindow.buttonWidth)
      }))
