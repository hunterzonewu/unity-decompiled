...
namespace UnityEditor
{
  internal class AssetStoreInstaBuyWindow : EditorWindow
  {
    ...
    public static AssetStoreInstaBuyWindow ShowAssetStoreInstaBuyWindow(AssetStoreAsset asset, string purchaseMessage, string paymentMethodCard, string paymentMethodExpire, string priceText)
    {
      ...
      Analytics.Track(string.Format("/AssetStore/ShowInstaBuy/{0}/{1}", (object) windowWithRect.m_Asset.packageID, (object) windowWithRect.m_Asset.id));
      return windowWithRect;
    }

    public static void ShowAssetStoreInstaBuyWindowBuilding(AssetStoreAsset asset)
    {
      AssetStoreInstaBuyWindow storeInstaBuyWindow = AssetStoreInstaBuyWindow.ShowAssetStoreInstaBuyWindow(asset, string.Empty, string.Empty, string.Empty, string.Empty);
      ...
      else
      {
        ...
        Analytics.Track(string.Format("/AssetStore/ShowInstaFree/{0}/{1}", (object) storeInstaBuyWindow.m_Asset.packageID, (object) storeInstaBuyWindow.m_Asset.id));
      }
    }
    ...
    private void PasswordGUI()
    {
      ...
      bool flag1 = this.m_PurchaseMessage != null && this.m_PurchaseMessage != string.Empty;
      ...
      if (GUILayout.Button("Just put to basket..."))
      {
        ...
        Analytics.Track(string.Format("/AssetStore/PutToBasket/{0}/{1}", (object) this.m_Asset.packageID, (object) this.m_Asset.id));
      ...
      {
        Analytics.Track(string.Format("/AssetStore/CancelInstaBuy/{0}/{1}", (object) this.m_Asset.packageID, (object) this.m_Asset.id));
        this.m_Asset = (AssetStoreAsset) null;
        this.Close();
        GUIUtility.ExitGUI();
      }
    ...
    private void PurchaseDeclinedGUI()
    {
      ...
      if (GUILayout.Button("Close"))
      {
        Analytics.Track(string.Format("/AssetStore/DeclinedAbort/{0}/{1}", (object) this.m_Asset.packageID, (object) this.m_Asset.id));
        this.m_Asset = (AssetStoreAsset) null;
        this.Close();
      }
      GUILayout.Space(5f);
      ...
      {
        AssetStore.Open(string.Format("content/{0}/basketpurchase", (object) this.m_Asset.packageID));
        Analytics.Track(string.Format("/AssetStore/DeclinedPutToBasket/{0}/{1}", (object) this.m_Asset.packageID, (object) this.m_Asset.id));
        this.m_Asset = (AssetStoreAsset) null;
        this.Close();
      }
      GUILayout.Space(5f);
