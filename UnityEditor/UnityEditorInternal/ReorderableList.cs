...
namespace UnityEditorInternal
{
  public class ReorderableList
  {
    ...
    public int count
    {
      get
      {
        ...
        foreach (UnityEngine.Object targetObject in this.m_Elements.serializedObject.targetObjects)
          val2 = Math.Min(new SerializedObject(targetObject).FindProperty(this.m_Elements.propertyPath).arraySize, val2);
        return val2;
      }
    }
    ...
    public void DoLayoutList()
    {
      ...
      Rect rect1 = GUILayoutUtility.GetRect(0.0f, this.headerHeight, new GUILayoutOption[1]{ GUILayout.ExpandWidth(true) });
      Rect rect2 = GUILayoutUtility.GetRect(10f, this.GetListElementHeight(), new GUILayoutOption[1]{ GUILayout.ExpandWidth(true) });
      Rect rect3 = GUILayoutUtility.GetRect(4f, this.footerHeight, new GUILayoutOption[1]{ GUILayout.ExpandWidth(true) });
    ...
    {
      return 0.0f + this.GetListElementHeight() + this.headerHeight + this.footerHeight;
    }
    ...
    private float GetListElementHeight()
    {
      ...
      if (this.elementHeightCallback != null)
        return (float) ((double) this.GetElementYOffset(count - 1) + (double) this.GetElementHeight(count - 1) + 7.0);
      return (float) ((double) this.elementHeight * (double) count + 7.0);
    }
    ...
    private void DoListElements(Rect listRect)
    {
      ...
      if ((this.m_Elements != null && this.m_Elements.isArray || this.m_ElementList != null) && count > 0)
      {
        if (this.IsDragging() && Event.current.type == EventType.Repaint)
        {
          ...
          for (int index = 0; index < this.m_NonDragTargetIndices.Count; ++index)
          {
            if (this.m_NonDragTargetIndices[index] != -1)
            {
              ...
              if (this.drawElementBackgroundCallback == null)
                ReorderableList.s_Defaults.DrawElementBackground(rect1, index, false, false, this.m_Draggable);
              else
                this.drawElementBackgroundCallback(rect1, index, false, false);
              ReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, index, false, false, this.m_Draggable);
              Rect contentRect = this.GetContentRect(rect1);
              if (this.drawElementCallback == null)
              {
                if (this.m_Elements != null)
                  ReorderableList.s_Defaults.DrawElement(contentRect, this.m_Elements.GetArrayElementAtIndex(this.m_NonDragTargetIndices[index]), (object) null, false, false, this.m_Draggable);
                else
                  ReorderableList.s_Defaults.DrawElement(contentRect, (SerializedProperty) null, this.m_ElementList[this.m_NonDragTargetIndices[index]], false, false, this.m_Draggable);
              }
              else
          ...
          if (this.drawElementBackgroundCallback == null)
            ReorderableList.s_Defaults.DrawElementBackground(rect1, this.m_ActiveElement, true, true, this.m_Draggable);
          else
            this.drawElementBackgroundCallback(rect1, this.m_ActiveElement, true, true);
          ReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, this.m_ActiveElement, true, true, this.m_Draggable);
          Rect contentRect1 = this.GetContentRect(rect1);
          if (this.drawElementCallback == null)
          {
            if (this.m_Elements != null)
              ReorderableList.s_Defaults.DrawElement(contentRect1, this.m_Elements.GetArrayElementAtIndex(this.m_ActiveElement), (object) null, true, true, this.m_Draggable);
            else
              ReorderableList.s_Defaults.DrawElement(contentRect1, (SerializedProperty) null, this.m_ElementList[this.m_ActiveElement], true, true, this.m_Draggable);
          }
          else
        ...
        else
        {
          for (int index = 0; index < count; ++index)
          {
            ...
            rect1.y = listRect.y + this.GetElementYOffset(index);
            if (this.drawElementBackgroundCallback == null)
              ReorderableList.s_Defaults.DrawElementBackground(rect1, index, flag1, flag2, this.m_Draggable);
            else
              this.drawElementBackgroundCallback(rect1, index, flag1, flag2);
            ReorderableList.s_Defaults.DrawElementDraggingHandle(rect1, index, flag1, flag2, this.m_Draggable);
            Rect contentRect = this.GetContentRect(rect1);
            if (this.drawElementCallback == null)
            {
              if (this.m_Elements != null)
                ReorderableList.s_Defaults.DrawElement(contentRect, this.m_Elements.GetArrayElementAtIndex(index), (object) null, flag1, flag2, this.m_Draggable);
              else
                ReorderableList.s_Defaults.DrawElement(contentRect, (SerializedProperty) null, this.m_ElementList[index], flag1, flag2, this.m_Draggable);
            }
            else
    ...
    private bool IsMouseInsideActiveElement(Rect listRect)
    {
      ...
      if (rowIndex == this.m_ActiveElement)
        return this.GetRowRect(rowIndex, listRect).Contains(Event.current.mousePosition);
      return false;
    }
    ...
    public class Defaults
    {
      ...
      public GUIStyle elementBackground = new GUIStyle((GUIStyle) "RL Element");
      ...
      public void DrawFooter(Rect rect, ReorderableList list)
      {
        ...
        if (list.displayAdd && GUI.Button(rect1, list.onAddDropdownCallback == null ? this.iconToolbarPlus : this.iconToolbarPlusMore, this.preButton))
        {
      ...
      public void DoAddButton(ReorderableList list)
      {
        ...
        else
        {
          ...
          if (elementType == typeof (string))
            list.index = list.list.Add((object) string.Empty);
