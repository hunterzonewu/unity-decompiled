...
namespace UnityEditorInternal.VersionControl
{
  public class Overlay
  {
    ...
    {
      if ((double) itemRect.width > (double) itemRect.height)
      {
        itemRect.x += 16f;
        itemRect.width = 20f;
      }
    ...
    public static void DrawOverlay(Asset asset, Rect itemRect)
    {
      ...
      if (externalVersionControl == ExternalVersionControl.Disabled || externalVersionControl == ExternalVersionControl.AutoDetect || (externalVersionControl == ExternalVersionControl.Generic || externalVersionControl == ExternalVersionControl.AssetServer))
        return;
      Overlay.DrawOverlays(asset, (Asset) null, itemRect);
    }
    ...
    private static void DrawMetaOverlay(Rect iconRect, bool isRemote)
    {
      ...
      {
        iconRect.x -= 5f;
        GUI.DrawTexture(iconRect, (Texture) Overlay.s_BlueLeftParan);
        iconRect.x += 8f;
        GUI.DrawTexture(iconRect, (Texture) Overlay.s_BlueRightParan);
      }
      else
      {
        iconRect.x -= 5f;
        GUI.DrawTexture(iconRect, (Texture) Overlay.s_RedLeftParan);
        iconRect.x += 8f;
        GUI.DrawTexture(iconRect, (Texture) Overlay.s_RedRightParan);
      }
    }
    ...
    {
      if (!((Object) Overlay.s_BlueLeftParan == (Object) null))
        return;
